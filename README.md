# LeetCode Solutions

![Tests](https://github.com/ikristina/leetcode/workflows/Tests/badge.svg)

Go solutions for LeetCode problems with comprehensive test coverage.

## Running Tests

### Go

```bash
go test ./...
```

### Python

To work with the Python files, you can use the virtual environment in the `problems/python_dump` directory:

```bash
cd problems/python_dump
python3 -m venv venv
source venv/bin/activate
# Run your scripts or install dependencies like pytest here
# Use 'deactivate' when you are done
```

## Problems Solved

- [1. Two Sum](problems/1-two-sum.go)
- [2. Add Two Numbers](problems/2-add-two-numbers.go)
- [3. Longest Substring Without Repeating Characters](problems/3-longest-substring-without-repeating-characters.go)
- [4. Median of Two Sorted Arrays](problems/4-median-of-two-sorted-arrays.go)
- [5. Longest Palindromic Substring](problems/5-longest-palindromic-substring.go)
- [11. Container With Most Water](problems/11-container-with-max-water.go)
- [13. Roman to Integer](problems/13-roman-to-integer.go)
- [14. Longest Common Prefix](problems/14-longest-common-prefix.go)
- [15. Three Sum](problems/15-three-sum.go)
- [17. Letter Combinations of a Phone Number](problems/17-letter-combinations-of-a-phone-number.go)
- [19. Remove Nth Node From End of List](problems/19-remove-nth-node-from-end-linked-list.go)
- [20. Valid Parentheses](problems/20-valid-parentheses.go)
- [23. Merge k Sorted Lists](problems/python_dump/p23_merge_k_sorted_lists.py) (Python)
- [26. Remove Duplicates from Sorted Array](problems/26-remove-duplicates.go)
- [27. Remove Element](problems/27-remove-element.go)
- [28. Find the Index of the First Occurrence in a String](problems/28-find-the-ind-of-first-occur.go)
- [33. Search in Rotated Sorted Array](problems/33-search-in-rotated-sorted-array.go)
- [33. Search in Rotated Sorted Array (Python)](problems/python_dump/p33_search_in_rotated_sorted_array.py)
- [36. Valid Sudoku](problems/36-valid-sudoku.go)
- [49. Group Anagrams](problems/49-group-anagrams.go)
- [54. Spiral Matrix](problems/54-spiral-matrix.go)
- [55. Jump Game](problems/55-jump-game.go)
- [56. Merge Intervals](problems/56-merge-intervals.go)
- [57. Insert Interval](problems/57-insert-interval.go)
- [58. Length of Last Word](problems/58-length-of-last-word.go)
- [70. Climbing Stairs](problems/70-climbing-stairs.go)
- [75. Sort Colors](problems/75-sort-colors.go)
- [80. Remove Duplicates from Sorted Array II](problems/80-remove-duplicates-2.go)
- [84. Largest Rectangle in Histogram](problems/84-largest-rectangle-in-histogram.go)
- [84. Largest Rectangle in Histogram (Python)](problems/python_dump/p84_largest_rectangle_area.py)
- [88. Merge Sorted Array](problems/88-merge-sorted-array.go)
- [98. Validate Binary Search Tree](problems/98-validate-binary-search-tree.go)
- [102. Binary Tree Level Order Traversal](problems/102-binary-tree-level-order-traversal.go)
- [102. Binary Tree Level Order Traversal (Python)](problems/python_dump/p102_binary_tree_level_order_traversal.py)
- [104. Maximum Depth of Binary Tree](problems/104-maximum-depth-of-binary-tree.go)
- [104. Maximum Depth of Binary Tree (Python)](problems/python_dump/p104_max_depth_binary_tree.py)
- [105. Construct Binary Tree from Preorder and Inorder Traversal](problems/105-construct-bt-preorder-inorder-traversal.go)
- [116. Populating Next Right Pointers in Each Node](problems/116-populating-next-right-pointers-in-each-node.go)
- [116. Populating Next Right Pointers in Each Node (Python)](problems/python_dump/p116_populating_next_right_pointers_in_each_node.py)
- [121. Best Time to Buy and Sell Stock](problems/121-stock-prices.go)
- [122. Best Time to Buy and Sell Stock II](problems/122-stock-prices.go)
- [124. Binary Tree Maximum Path Sum](problems/124-binary-tree-maximum-path-sum.go)
- [125. Valid Palindrome](problems/125-valid-palindrome.go)
- [128. Longest Consecutive Sequence](problems/128-longest-consecutive-sequence.go)
- [133. Clone Graph](problems/133-clone-graph.go)
- [138. Copy List with Random Pointer](problems/138-copy-list-with-random-pointer.go)
- [141. Linked List Cycle](problems/141-linked-list-cycle.go)
- [150. Evaluate Reverse Polish Notation](problems/150-evaluate-reverse-polish-notation.go)
- [152. Maximum Product Subarray](problems/152-maximum-product-subarray.go)
- [153. Find Minimum in Rotated Sorted Array](problems/153-find-minimum-in-rotated-sorted-array.go)
- [155. Min Stack](problems/155-min-stack.go)
- [167. Two Sum II - Input Array Is Sorted](problems/167-two-sum-ii.go)
- [169. Majority Element](problems/169-majority-element.go)
- [189. Rotate Array](problems/189-rotate-array.go)
- [198. House Robber (Python)](problems/python_dump/p198_house_robber.py)
- [199. Binary Tree Right Side View](problems/199-right-side-view.go)
- [200. Number of Islands](problems/200-number-of-islands.go)
- [207. Course Schedule](problems/207-course-schedule.go)
- [207. Course Schedule (Python)](problems/python_dump/p207_course_schedule.py)
- [209. Minimum Size Subarray Sum](problems/209-minimum-size-subarray-sum.go)
- [214. Pacific Atlantic Water Flow](problems/214-pacific-atlantic-water-flow.go)
- [215. Kth Largest Element in an Array](problems/215-Kth-largest-element-in-an-array.go)
- [226. Invert Binary Tree](problems/226-invert-binary-tree.go)
- [228. Summary Ranges](problems/228-summary-ranges.go)
- [230. Kth Smallest Element in a BST](problems/230-kth-smallest-element-bst.go)
- [235. Lowest Common Ancestor of a Binary Search Tree](problems/235-lowest-common-ancestor-of-bst.go)
- [242. Valid Anagram](problems/242-valid-anagram.go)
- [261. Graph Valid Tree](problems/261-graph-valid-tree.go)
- [269. Alien Dictionary](problems/269-alien-dictionary.go)
- [269. Alien Dictionary (Kahn's)](problems/269-alien-dictionary-kahn.go)
- [287. Find the Duplicate Number](problems/287-find-the-duplicate-number.go)
- [295. Max Area of Island](problems/295-max-area-of-island.go)
- [322. Coin Change](problems/322-coin-change.go)
- [322. Coin Change (Python)](problems/python_dump/p322_coin_change.py)
- [362. Design Counter](problems/362-design-counter.go)
- [380. Insert Delete GetRandom O(1)](problems/380-insert-delete-getrandom-o1.go)
- [383. Ransom Note](problems/383-ransom-note.go)
- [392. Is Subsequence](problems/392-is-subsequence.go)
- [394. Decode String](problems/394-decode-string.go)
- [469. Next Greater Element I](problems/469-next-greater-element-i.go)
- [543. Diameter of Binary Tree](problems/543-diameter-of-binary-tree.go)
- [572. Subtree of Another Tree](problems/572-subtree-or-another-tree.go)
- [621. Task Scheduler](problems/621-task-scheduler.go)
- [621. Task Scheduler (Python)](problems/python_dump/p621_task_scheduler.py)
- [637. Average of Levels in Binary Tree](problems/637-average-of-levels-in-binary-tree.go)
- [704. Binary Search](problems/704-binary-search.go)
- [739. Daily Temperatures](problems/739-daily-temperatures.go)
- [743. Network Delay Time](problems/743-network-delay-time.go)
- [743. Network Delay Time (Priority Queue)](problems/743-network-delay-time-pq.go)
- [973. K Closest Points to Origin](problems/973-k-closest-points-to-origin.go)
- [973. K Closest Points to Origin (Python)](problems/python_dump/p973_k_closest_points_to_origin.py)
- [981. Time Based Key-Value Store](problems/981-time-based-key-value-store.go)
- [994. Rotting Oranges](problems/994-rotting-oranges.go)
- [1046. Last Stone Weight](problems/1046-last-stone-weight.go)
- [1143. Longest Common Subsequence](problems/1143-longest-common-subsequence.go)
- [1664. Ways to Make a Fair Array](problems/1664-ways-to-make-a-fair-array.go)
- [2130. Maximum Twin Sum of A Linked List](problems/2130-maximum-twin-sum-of-a-linked-list.go)

## Data Structures

- [Trie](data_structures/trie.go)
- [Binary Tree](data_structures/tree.go)
